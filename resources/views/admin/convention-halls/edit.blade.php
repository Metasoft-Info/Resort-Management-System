...
@section('title', 'Edit Convention Hall')
@section('header', 'Edit Convention Hall')

@section('content')
<div class="max-w-4xl">
    <form action="{{ route('admin.convention-halls.update', $conventionHall) }}" method="POST" class="bg-white rounded-2xl shadow-xl p-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="md:col-span-2">
                ...
                <input type="text" name="name" value="{{ old('name', $conventionHall->name) }}" 
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="4" 
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary-500 transition">{{ old('description', $conventionHall->description) }}</textarea>
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="dimensions" value="{{ old('dimensions', $conventionHall->dimensions) }}" 
            ...
            <div>
                ...
                <input type="number" name="max_capacity" value="{{ old('max_capacity', $conventionHall->max_capacity) }}" 
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary-500 transition" 
                    required>
            </div>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="price_per_day" value="{{ old('price_per_day', $conventionHall->price_per_day) }}" 
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary-500 transition" 
                    required>
            </div>
            ...
            <div>
                ...
                <select name="status" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:outline-none focus:border-primary-500 transition" required>
                    <option value="available" {{ old('status', $conventionHall->status) == 'available' ? 'selected' : '' }}>Available</option>
                    <option value="booked" {{ old('status', $conventionHall->status) == 'booked' ? 'selected' : '' }}>Booked</option>
                    <option value="maintenance" {{ old('status', $conventionHall->status) == 'maintenance' ? 'selected' : '' }}>Maintenance</option>
                </select>
            </div>
        </div>
        ...
        <div class="flex gap-4 mt-8 pt-6 border-t">
            <button type="submit" class="px-8 py-3 bg-gradient-to-r from-primary-600 to-accent-600 text-white rounded-xl hover:from-primary-700 hover:to-accent-700 transition font-semibold shadow-lg inline-flex items-center">
                <i class="fas fa-save mr-2"></i>Update Hall
            </button>
