...
<div class="p-6">
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('admin.addon-services.create') }}" class="bg-gradient-to-r from-primary-600 to-primary-700 text-white px-6 py-3 rounded-lg hover:from-primary-700 hover:to-primary-800 transition shadow-lg">
    ...
    <div class="flex gap-2 mb-6">
        <a href="{{ route('admin.addon-services.index') }}" class="px-4 py-2 rounded-lg font-semibold transition {{ !request('type') ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
            <i class="fas fa-list mr-2"></i>সব ({{ \App\Models\AddonService::count() }})
        </a>
        <a href="{{ route('admin.addon-services.index', ['type' => 'room']) }}" class="px-4 py-2 rounded-lg font-semibold transition {{ request('type') == 'room' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
        ...
        <a href="{{ route('admin.addon-services.index', ['type' => 'both']) }}" class="px-4 py-2 rounded-lg font-semibold transition {{ request('type') == 'both' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300' }}">
                ...
                <tr class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 text-center">
                        @if($service->is_active)
                            <span class="px-2 py-1 rounded-full text-xs font-semibold bg-primary-100 text-primary-800">সক্রিয়</span>
                        @else
                            <span class="px-2 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-800">নিষ্ক্রিয়</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-right">
                        <a href="{{ route('admin.addon-services.edit', $service) }}" class="text-primary-600 hover:text-primary-800 mr-3"><i class="fas fa-edit"></i></a>
