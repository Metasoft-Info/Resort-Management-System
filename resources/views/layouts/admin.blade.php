...
<body class="bg-gray-50">
 ...
 <h3 class="text-white font-bold"><i class="fas fa-bell mr-2"></i>Notifications</h3>
 <button onclick="markAllRead()" class="text-white/80 hover:text-white text-xs">Mark all read</button>
 </div>
 ...
 // Scroll active menu into view on load
 setTimeout(function() {
 const activeItem = document.querySelector('.sidebar-menu-item.bg-gradient-to-r');
 if (activeItem) {
...
function toggleSection(sectionKey) {
 ...
 const scrollTop = sidebarNav.scrollTop + (elementRect.top - navRect.top) - (navRect.height / 3);
 ...
 <h3 class="text-2xl font-bold text-gray-800 mb-3">Warning!</h3>
 ...
 <h3 class="text-2xl font-bold text-gray-800 mb-3">Confirm</h3>
 ...
 <i class="fas fa-check mr-2"></i>Yes, Confirmed
 </button>
 </div>
 </div>
 </div>
 ...
 showConfirmModal(message || 'Do you want to delete?', function() {
 form.submit();
 });
 return false;
 }
 ...
 if (response.success) {
 showGlobalModal('success', response.message || 'Operation successful!');
 } else if (response.error) {
 showGlobalModal('error', response.message || response.error || 'Something went wrong!');
 }
 }
 ...
 document.getElementById('notificationList').innerHTML = '<div class="p-4 text-center text-red-500"><i class="fas fa-exclamation-circle mr-2"></i>Load failed</div>';
 }
 }
 ...
 container.innerHTML = '<div class="p-6 text-center text-gray-400"><i class="fas fa-bell-slash text-3xl mb-2"></i><p class="text-sm">No notifications</p></div>';
 return;
 }
